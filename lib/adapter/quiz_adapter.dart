...
class QuizAdapter extends ChangeNotifier {
  ...
  Future<void> addQuiz(QuizModel quiz, BuildContext context) async {
    DocumentReference docRef =
        await firebaseFirestore.collection('Quizes').add(quiz.toMap());
    quiz.id = docRef.id;
  ...
  Future<void> addQuestionToQuiz(
      QuestionModel question, String id, BuildContext context) async {
    questions.add(question.qid);
    await firebaseFirestore
        .collection("Quizes")
        .doc(id)
        .update({"questions": FieldValue.arrayUnion(questions)});
    notifyListeners();
  }
